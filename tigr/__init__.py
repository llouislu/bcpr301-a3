...
def run(args):
    file_name = None
    if args.interactive:
        from .lib.source_reader.prompt_source_reader import \
            PromptSourceReader as Reader
    else:
        from .lib.source_reader.file_source_reader import \
            FileSourceReader as Reader
...
def main():
    ...
    group.add_argument('-i', '--interactive',
                       ...
                       help='go to interactive mode is this flag is set')
    ...
    group.add_argument('-p', '--parser', action='store',
                       help='specify a parser', choices=['regex', 'peg'],
                       default='peg')
    group.add_argument('-d', '--drawer', action='store',
                       help='specify a drawer', choices=['tkinter', 'turtle'],
                       default='turtle')
    group.add_argument('-pn', '--pen', action='store',
                       help='specify a pen number',
                       choices=[str(i) for i in range(1, 11)], default='1')
    group.add_argument('infile', nargs='?', type=argparse.FileType('r'),
                       default=sys.stdin,
                       help='input script file or stream from stdin')
